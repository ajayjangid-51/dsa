...
extern "C++"
{
    namespace std _GLIBCXX_VISIBILITY(default)
    {
        ...
        sqrt(float p1)
        {
            return __builtin_sqrtf(p1);
        }

        inline _GLIBCXX_CONSTEXPR long double
        // as yaha pe yeh 3-overloaded sqrt function hai , thus mtlb apne input k according particular sqrt-fn chal jayega.
        sqrt(long double p1)
        {
            return __builtin_sqrtl(p1);
        }
#endif
...
#ifndef __CORRECT_ISO_CPP11_MATH_H_PROTO_INT
        template <typename _Tp, typename _Up>
        constexpr typename __gnu_cxx::__enable_if<(__is_arithmetic<_Tp>::__value && __is_arithmetic<_Up>::__value), bool>::__type
...
#endif
        // as yaha pe jo apnko #ifndef and #endif dikh rahe hai yeh sab pre-processor-Statements hi hai. thus mtlb ess file meh kafi kuch pre-processing meh ho jata hai.
